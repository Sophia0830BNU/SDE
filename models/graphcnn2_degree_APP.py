...
class GraphCNN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, num_views,  input_dim, hidden_dim, output_dim, final_dropout, learn_eps, graph_pooling_type, neighbor_pooling_type, device):
        '''
        ...
        #self.S_list = self.assign_matrix()
        ...
        for layer in range(self.num_layers): #self.num_layers
            if layer == 0:
                self.linears_prediction.append(nn.Linear(hidden_dim, output_dim)) # input layer  0-th iteration  h0
            else:
    ...
    def next_layer_eps2(self, h, layer, padded_neighbor_list = None, Adj_block = None):
        ...
        if self.neighbor_pooling_type == "max":
            ...
            if self.neighbor_pooling_type == "average":
                ...
                pooled = pooled/degree # element-wise div
        pooled = pooled + (2 +self.eps2[layer])*h
    ...
    def CtoD(self, h, node_entropy):
        ...
        sorted_degrees, sorted_indices = torch.sort(node_entropy, descending=True)
        ...
        #    h = self.branches[i](h)
        #    discretized = torch.zeros_like(h).to(self.device)
        #    discretized[top_nodes_indices] = F.gumbel_softmax(F.softmax(h[top_nodes_indices], dim=1), dim=1, tau=0.47)
        #    discretized[~top_nodes_indices] = h[~top_nodes_indices]
        #    h_discrete.append(discretized)
        
        # h = torch.stack(h_discrete, dim=1)
        
        # h, att = self.attention(h)  
           

        h_discrete = torch.zeros_like(h).to(self.device)
        h_discrete[top_nodes_indices] = F.gumbel_softmax(F.softmax(h[top_nodes_indices], dim=1), dim=1, tau=0.47)
        h_discrete[~top_nodes_indices] = h[~top_nodes_indices]
        
        
        
        #S = self.S_list[layer].to(self.device)
        #return torch.matmul(h, S.to(torch.float))
        # h = torch.matmul(h, S)
        # if self.training:
    ...
    #     S_list = []
    #     for layer in range(self.num_layers-1):
    #         S = torch.zeros(self.hidden_dim, self.hidden_dim, dtype = torch.int)
    #         for i in range(self.hidden_dim):
    #             S[i, torch.randint(0, self.hidden_dim, (1,))] = 1
    #         S_list.append(S)
    ...
    def forward(self, batch_graph):
        ...
        #degree = torch.cat([torch.tensor(graph.degree_list) for graph in batch_graph], 0).to(self.device)
        #hidden_rep = [X_concat] 
        #h1 = X_concat
        ...
        for layer in range(self.num_layers-1):
            ...
            #print(attn.shape)
            #print(h_total.shape)
            h2 = self.alpha[layer] * h0 + (1-self.alpha[layer]) * h2
            
            hidden_rep.append(h2)
            
            h2 = self.CtoD(h2, node_entropy)
            alpha.append(h2)
