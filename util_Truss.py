...
def entropy(adj):
    ...
    return node_entropy, norm_adj_js   
...
def compute_edge_trussness(graph):
    ...
    # Sort edges by support in non-decreasing order
    ...
    while edges_sorted:
        for e in edges_sorted:
            u, v = e
            if supg[e] <= k - 2:
                # Update support for neighboring edges
                ...
                # Assign trussness value
                GT[u][v]['trussness'] = k - 1
                GT.remove_edge(u, v)

        # Update the edge list and increment k
...
def TGS(graph, delta=3, eta=3):
    ...
    for u, v, t_value in EH:
        ...
        # Check if TN(E) exceeds the threshold delta
        if TN_E >= delta:
            GS.remove_edge(u, v)
            # Update trussness after edge removal
...
def load_data(dataset, degree_as_tag):
    ...
    with open('dataset/dataset/%s/%s.txt' %(dataset, dataset), 'r') as f:
        ...
        for graph in g_list:
            ...
            # graph.node_entropy, graph.adj_js = entropy_k(adj)
...
if __name__ =="__main__":
    ...
    graphs, num_classes = load_data("COLLAB", degree_as_tag=False)
